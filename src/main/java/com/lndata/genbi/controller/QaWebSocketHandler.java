...
import com.lndata.genbi.statemachine.QueryStateMachine;
...
public class QaWebSocketHandler extends TextWebSocketHandler {
    ...
    protected void handleTextMessage(WebSocketSession session, TextMessage message) throws Exception {
        ...
        try {
            ...
            // 落庫：儲存問答記錄 + 自動建立 session
            saveAnswerToDb(question, answer);
    ...
    /** 將問答結果寫入 DB，讓 session/history 可用 */
    private void saveAnswerToDb(Question question, Answer answer) {
        try {
            String answerJson = objectMapper.writeValueAsString(answer);
            String sql = answer.getSqlSearchResult() != null
            ...
            sessionService.saveMessage(
                    ...
                    question.query(),
                    answer.getQueryRewrite(),
                    answer.getQueryIntent(),
                    sql,
                    answerJson,
                    question.bedrockModelId()
            );
            log.info("[QaWebSocket] 問答記錄已儲存: session={}, user={}", question.sessionId(), question.userId());
        } catch (Exception e) {
            log.error("[QaWebSocket] 儲存問答記錄失敗: {}", e.getMessage(), e);
        }
    }
